 ...
 */
class CardGateprzelewy24 extends \Opencart\System\Engine\Model {

    public function getMethods( $address ) {
        $this->load->language( 'extension/cardgate/payment/cardgateprzelewy24' );
        ...
        if ($this->config->get('payment_cardgateprzelewy24_custom_payment_method_text')){
            $title = trim($this->config->get('payment_cardgateprzelewy24_custom_payment_method_text'));
        } else {
        ...
        $method_data = [];
        if ( $status ) {
            $option_data['cardgateprzelewy24'] = [
                'code' => 'cardgateprzelewy24.cardgateprzelewy24',
