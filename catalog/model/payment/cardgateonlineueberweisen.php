...
class CardGateOnlineueberweisen extends \Opencart\System\Engine\Model {
    ...
    public function getMethod( $address ) {
        ...
        if ( !$this->config->get( 'payment_cardgateonlineueberweisen_geo_zone_id' ) ) {
            $status = true;
        ...
        if ($this->config->get('payment_cardgateonlineueberweisen_custom_payment_method_text')){
            $payment_text = trim($this->config->get('payment_cardgateonlineueberweisen_custom_payment_method_text'));
        } else {
            $payment_text = trim($this->language->get( 'text_title' ));
        }

        if ($this->config->get('payment_cardgate_use_logo') == 1) {
            $payment_logo = '<img style="max-height: 30px; max-width: 40px;" src="image/payment/cgp/onlineueberweisen.svg">&nbsp&nbsp';
