...
class CardgateGeneric extends \Opencart\System\Engine\Controller {
	...
	public function _confirm($payment) {
		...
		try { 
            ...
            $address = $this->getPaymentAddress($order_info);
			$oConsumer->address ()->setFirstName ( $address ['payment_firstname'] );
			$oConsumer->address ()->setLastName ( $address ['payment_lastname'] );
            $oConsumer->address ()->setAddress ( $address['payment_address_1'] );
			$oConsumer->address ()->setZipCode ($address['payment_postcode'] );
			$oConsumer->address ()->setCity ( $address ['payment_city'] );
			$oConsumer->address ()->setCountry ( $address ['payment_iso_code_2'] );
			
			if ($this->cart->hasShipping ()) {
				
				$oConsumer->shippingAddress ()->setFirstName ( $address ['shipping_firstname'] );
				$oConsumer->shippingAddress ()->setLastName ( $address ['shipping_lastname'] );
				$oConsumer->shippingAddress ()->setAddress ( $address ['shipping_address_1'] );
				$oConsumer->shippingAddress ()->setZipCode ( $address ['shipping_postcode'] );
				$oConsumer->shippingAddress ()->setCity ( $address ['shipping_city'] );
				$oConsumer->shippingAddress ()->setCountry ( $address ['shipping_iso_code_2'] );
			}
			...
			if (NULL !== $sActionUrl) {
				...
				$data ['status'] = 'failed';
				$data ['error'] = 'The transaction failed at CardGate.';
			}
    ...
    private function getPaymentAddress($data){
        $keys = array('firstname','lastname','address_1','address_2','postcode','city','country_id');
        $address = [];
        foreach ($keys as $key){
            $address['payment_'.$key] = (is_null($data['payment_'.$key])? $data['shipping_'.$key] : $data['payment_'.$key]);
            $address['shipping_'.$key] = $data['shipping_'.$key];
        }
        $address['payment_address_1'] .= is_null($data['payment_address_2']) ? '':' '.$data['payment_address_2'];
        $address['payment_iso_code_2'] = $this->getCountryCode($data['payment_country_id']);
        $address['shipping_iso_code_2'] = $this->getCountryCode($data['shipping_country_id']);
        return $address;
    }
